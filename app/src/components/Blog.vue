<template>
  <div class="store-grid">
    ...
    >
      ...
      <p class="tm-pt-30" >
        {{ item.Pera.slice(0, 200) || item.Pera.replace(/'<br>'/gi,'') }}
      </p>
...
export default {
  ...
  methods: {
    ...
    async lodData() {
      let data = await axiOs.get('http://127.0.0.1:8080/allBlog')
      console.log(data);
      this.BlogData = data.data
      this.BlogData.Pera.replace(/<br>/gi,'');

    },


    async SData(){
      if((this.Data != this.nData)  ){
      this.nData = this.Data
      console.log(this.Data);

      let data = await axiOs.get(`http://127.0.0.1:8080/Search/${this.Data}`)
      // console.log(data)
